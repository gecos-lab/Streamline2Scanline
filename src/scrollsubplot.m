...
elseif(isempty(position))
  ...
  if (min(thisPlot) < 1)&&0 % negative Thisplot corresponds to
    ...
    % PERC_OFFSET_L = 0.07; % original setting in SCROLLSUBPLOT (less space between plots)
    PERC_OFFSET_L = 2*0.09; % original setting in SUBPLOT (more space between plots)
    % PERC_OFFSET_R = 0.085; % original setting in SCROLLSUBPLOT (less space between plots)
    PERC_OFFSET_R = 2*0.045; % original setting in SCROLLSUBPLOT (more space between plots)
